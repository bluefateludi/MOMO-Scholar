# MOMO Scholar MVP Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a CLI-first MOMO Scholar MVP that retrieves papers, normalizes metadata, extracts evidence, generates citation-traceable mini surveys, and renders lightweight evaluation/showcase artifacts.
...
- `docs/superpowers/specs/2026-07-10-momo-scholar-mvp-design.md`
...
| File | Responsibility |
|:---|:---|
| `momo_scholar/schemas.py` | Pydantic models for Paper, Chunk, Evidence, Claim, Report, EvalResult |
| `momo_scholar/config.py` | Environment/config parsing with safe defaults |
| `momo_scholar/io.py` | Output directory creation and JSON/JSONL/Markdown writes |
| `momo_scholar/retrieval/arxiv.py` | arXiv API query and XML parsing |
| `momo_scholar/retrieval/normalize.py` | Normalize and deduplicate paper metadata |
| `momo_scholar/text/loader.py` | Load PDF text or fallback abstract text |
| `momo_scholar/text/chunker.py` | Convert text into traceable chunks |
| `momo_scholar/evidence/retriever.py` | Rank chunks as evidence for a research question |
| `momo_scholar/evidence/citation_checker.py` | Validate that report claims have evidence IDs |
| `momo_scholar/llm/*` | LLM abstraction; mock provider for tests; OpenAI-compatible provider later |
| `momo_scholar/synthesis/*` | Structure paper analyses and generate survey draft |
| `momo_scholar/rendering/*` | Render Markdown and static HTML |
| `momo_scholar/eval/*` | Compute lightweight MVP metrics |
| `momo_scholar/pipeline.py` | Orchestrate modules into one run |
| `momo_scholar/cli.py` | CLI entrypoint |
...
```bash
momo-scholar run "LLM agents for scientific literature review" --limit 3 --no-pdf
```
...
```bash
python -m momo_scholar.cli run "LLM agents for scientific literature review" --limit 3 --no-pdf
```
...
- Create: `momo_scholar/__init__.py`
- Create: `momo_scholar/cli.py`
- Create: `momo_scholar/config.py`
...
name = "momo-scholar"
version = "0.1.0"
description = "CLI-first Scholar Research agent with citation-traceable reports"
...
[project.scripts]
momo-scholar = "momo_scholar.cli:app"
...
app = typer.Typer(help="MOMO Scholar: citation-traceable paper survey assistant")
...
def run(
    ...
    typer.echo(f"MOMO Scholar MVP placeholder: {question} limit={limit} no_pdf={no_pdf}")
...
```bash
python -m momo_scholar.cli run "test query" --limit 2 --no-pdf
```

Expected:

```text
MOMO Scholar MVP placeholder: test query limit=2 no_pdf=True
```
...
```bash
git add pyproject.toml momo_scholar/__init__.py momo_scholar/cli.py
git commit -m "chore: scaffold MOMO Scholar package"
```
...
- Create: `momo_scholar/schemas.py`
...
```python
from momo_scholar.schemas import Chunk, Evidence, Paper, ReportClaim
...
```text
ModuleNotFoundError: No module named 'momo_scholar.schemas'
```
...
```bash
git add momo_scholar/schemas.py tests/test_schemas.py
git commit -m "feat: add core MOMO Scholar schemas"
```
...
from momo_scholar.io import create_run_dir, write_json
...
```text
ModuleNotFoundError: No module named 'momo_scholar.io'
```
...
def slugify(value: str, max_length: int = 60) -> str:
    ...
    return slug[:max_length].strip("-") or "momo-scholar-run"
...
- Create: `momo_scholar/retrieval/__init__.py`
- Create: `momo_scholar/retrieval/arxiv.py`
...
<feed xmlns="http://www.w3.org/2005/Atom">
  <entry>
    ...
    <title>Example MOMO Scholar Study</title>
    <summary>This paper studies retrieval augmented MOMO Scholars.</summary>
...
from momo_scholar.retrieval.arxiv import parse_arxiv_feed
...
def test_parse_arxiv_feed_extracts_paper_fields():
    ...
    paper = papers[0]
    assert paper.paper_id == "arxiv:2401.00001"
    assert paper.title == "Example MOMO Scholar Study"
...
from momo_scholar.schemas import Paper
...
- Create: `momo_scholar/retrieval/normalize.py`
...
```python
from momo_scholar.retrieval.normalize import dedupe_papers
from momo_scholar.schemas import Paper
...
- Create: `momo_scholar/pipeline.py`
- Create: `momo_scholar/rendering/__init__.py`
- Create: `momo_scholar/rendering/markdown.py`
- Modify: `momo_scholar/cli.py`
...
from momo_scholar.pipeline import run_pipeline
from momo_scholar.schemas import Paper
...
def test_pipeline_writes_papers_and_report(tmp_path):
    def fake_search(query: str, limit: int):
        return [
            Paper(
                paper_id="arxiv:2401.00001",
                title="Example MOMO Scholar Study",
                authors=["Alice Researcher"],
                year=2024,
                abstract="This paper studies retrieval augmented MOMO Scholars.",
                url="https://arxiv.org/abs/2401.00001",
                pdf_url=None,
                source="arxiv",
            )
    ...
    assert "Example MOMO Scholar Study" in report
```
...
```text
ModuleNotFoundError: No module named 'momo_scholar.pipeline'
```
...
from momo_scholar.io import create_run_dir, write_json, write_text
from momo_scholar.rendering.markdown import render_initial_review
from momo_scholar.retrieval.arxiv import search_arxiv
from momo_scholar.retrieval.normalize import dedupe_papers
from momo_scholar.schemas import Paper
...
- Create: `momo_scholar/text/__init__.py`
- Create: `momo_scholar/text/loader.py`
- Create: `momo_scholar/text/chunker.py`
...
```text
Abstract
MOMO Scholars retrieve scientific documents and generate grounded summaries.
...
```python
from momo_scholar.text.chunker import chunk_text
...
from momo_scholar.schemas import Chunk
...
- Create: `momo_scholar/evidence/__init__.py`
- Create: `momo_scholar/evidence/retriever.py`
...
```python
from momo_scholar.evidence.retriever import retrieve_evidence
from momo_scholar.schemas import Chunk
...
def test_retrieve_evidence_ranks_matching_chunks():
    chunks = [
        Chunk(
            ...
            paper_id="p1",
            text="MOMO Scholars use retrieval and source grounding.",
            token_count=7,
        ),
        Chunk(
    ...
    evidence = retrieve_evidence("retrieval grounding for MOMO Scholars", chunks, top_k=1)
...
from momo_scholar.schemas import Chunk, Evidence
...
- Create: `momo_scholar/synthesis/__init__.py`
- Create: `momo_scholar/synthesis/paper_reader.py`
- Create: `momo_scholar/synthesis/survey.py`
- Create: `momo_scholar/evidence/citation_checker.py`
- Create: `tests/test_citation_checker.py`
- Modify: `momo_scholar/pipeline.py`
- Modify: `momo_scholar/rendering/markdown.py`
...
```python
from momo_scholar.evidence.citation_checker import check_claims
from momo_scholar.schemas import Evidence, ReportClaim
...
from momo_scholar.schemas import Evidence, Paper, PaperAnalysis
...
from momo_scholar.schemas import Evidence, Paper, PaperAnalysis, ReportClaim
...
Modify `momo_scholar/rendering/markdown.py` to add:

```python
from momo_scholar.schemas import Evidence, Paper, ReportClaim
...
Modify `momo_scholar/pipeline.py` to:

```python
from momo_scholar.evidence.citation_checker import check_claims
from momo_scholar.evidence.retriever import retrieve_evidence
from momo_scholar.rendering.markdown import render_evidence_review
from momo_scholar.synthesis.paper_reader import analyze_paper
from momo_scholar.synthesis.survey import synthesize_claims
from momo_scholar.text.chunker import chunk_text
from momo_scholar.text.loader import load_abstract_text
```
...
- Create: `momo_scholar/eval/__init__.py`
- Create: `momo_scholar/eval/metrics.py`
- Create: `momo_scholar/eval/runner.py`
...
```python
from momo_scholar.eval.metrics import evidence_coverage, unsupported_claim_rate
from momo_scholar.schemas import ReportClaim
...
from momo_scholar.eval.metrics import evidence_coverage, unsupported_claim_rate
from momo_scholar.schemas import EvalSummary, ReportClaim
...
```python
from momo_scholar.eval.runner import render_eval_markdown, summarize_eval
```
...
- Create: `momo_scholar/rendering/html.py`
...
```python
from momo_scholar.rendering.html import render_html
...
<head>
  ...
  <title>MOMO Scholar Report</title>
  <style>
...
CLI-first Scholar Research agent that retrieves scientific papers, extracts evidence, and generates citation-traceable mini surveys.
...
```bash
git add .
git commit -m "feat: build MOMO Scholar mvp vertical slice"
```
