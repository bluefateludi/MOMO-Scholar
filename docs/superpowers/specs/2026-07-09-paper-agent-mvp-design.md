...
Paper Agent 是一个面向简历展示的论文研读与综述助手。第一版不追求“大而全的科研平台”，而是聚焦一个可讲深、可运行、可展示的核心闭环：
...
| 能力 | MVP 做法 | 说明 |
|:---|:---|:---|
| CLI 入口 | `paper-agent run "<research question>"` | 第一版 CLI-first，降低 UI 成本 |
...
> Built a CLI-first paper research agent that retrieves scientific papers, parses PDFs, extracts evidence-grounded claims, and generates citation-traceable literature reviews with lightweight evaluation for retrieval quality and citation faithfulness.
...
2. 项目名称：继续叫 Paper Agent，还是改成 PaperTrail Agent / ScholarTrace。
3. 是否初始化 Git 仓库，便于后续按 milestone commit。
