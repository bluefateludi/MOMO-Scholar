...
- 扩展 Evaluation Control，比较 Recall@K、Precision@K、MRR@K、nDCG@K。
...
- MRR@K：Top-K 内首个相关 chunk 的倒数排名；无命中为 0。
...
- `EvidenceRetrievalService.retrieve(question, chunks, run_id, event_sink=None)` 返回 Evidence 与 diagnostics；pipeline 后续阶段只消费 Evidence。
...
- grade `> 0` 视为 Recall@K/Precision@K/MRR@K 的相关结果；grade `0` 仅用于显式非相关标注。
...
- 成功时在返回前成功交付且只交付一个 `status="ok"` 终态事件；sink 自身失败时不保证终态事件已交付。
...
`RetrievalSourceUnavailable` 必须携带稳定 `degradation_code`，不得通过异常字符串分类。错误事件至少包含 `failure_stage`（`validation|lexical|vector_index|vector_query|fusion|evidence_conversion`）以及当时已知的候选计数；尚未开始的阶段计数为 0。question、run ID 或 K 参数校验失败使用 `validation`。
