...
- [GPT Researcher](https://github.com/assafelovic/gpt-researcher)：开源 deep research Agent，适合参考 planner、researcher、publisher 的研究报告生成 pipeline。
- [STORM](https://github.com/stanford-oval/storm)：偏通用知识调研、提纲规划与长报告生成，适合参考 outline-first 的 report generation 流程。
- [OpenScholar](https://github.com/AkariAsai/OpenScholar)：专门面向科学文献检索与综合，和本项目目标最接近，适合参考 scientific RAG、reranking 与 citation attribution。
- [PaperQA](https://arxiv.org/abs/2312.07559)：面向科学论文的全文检索与证据综合系统，适合参考 source relevance、evidence synthesis 与 LitQA 风格评测。
- [Paper2Agent](https://github.com/jmiao24/Paper2Agent)：把论文代码库和教程转成 MCP tools 与可交互科研 Agent，适合参考工具化产物、benchmark、coverage 和 quality report。
- [Sakana AI: The AI Scientist](https://github.com/SakanaAI/AI-Scientist)：自动化科研流程项目，适合了解研究型 Agent 的边界、成本、失败模式和风险控制。

### 数据源与 API

- [arXiv API](https://info.arxiv.org/help/api/index.html)：用于论文检索、基础元数据获取和开放 PDF 入口。
- [Semantic Scholar API](https://api.semanticscholar.org/api-docs/)：用于论文、作者、引用、相关论文和影响力信息。
- [OpenAlex API](https://docs.openalex.org/)：开放学术图谱，适合做无 API key 的备用检索源和元数据补全。

### 本项目取舍

本项目不复刻上述任一系统，而是聚焦一个月内可完成的简历级 MVP：围绕研究问题检索论文、解析 PDF、抽取证据、生成可追溯综述，并提供轻量评测报告。第一版优先做 CLI-first 的核心 pipeline，最后补一个极简展示页；MCP、复杂多 Agent 协作、自动化实验和大规模 benchmark 放到后续扩展。
