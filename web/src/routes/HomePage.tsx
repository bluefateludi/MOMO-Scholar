...
export function HomePage() {
  ...
  return <>
    <section className="tech-hero"><div><p className="eyebrow">Wave 1 · evidence-grounded component research</p><h1>Choose with<br/><em>receipts.</em></h1><p className="dek">Compare Python AI components against hard constraints, official evidence, and allowlisted local checks.</p></div><aside className="scope-card"><b>V1 support boundary</b><strong>Local RAG vector stores</strong><p>Chroma and Qdrant Local may cross the PoC gate. Unknown recipes stay research-only.</p></aside></section>
    <section className="tech-desk"><form className="tech-form" onSubmit={submit}><header><span>01</span><div><p className="eyebrow">New task</p><h2>Frame the decision</h2></div></header>
      ...
      <fieldset className="mode-field"><legend>Mode</legend><label><input type="radio" name="mode" checked={(form.mode ?? "fast") === "fast"} onChange={() => setForm({ ...form, mode: "fast" })}/> Fast</label><label><input type="radio" name="mode" checked={form.mode === "verified"} onChange={() => setForm({ ...form, mode: "verified" })}/> Verified</label></fieldset>
