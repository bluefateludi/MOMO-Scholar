...
export function HomePage() {
  ...
  return <>
    ...
    <section className="tech-desk"><form className="tech-form" onSubmit={submit}><header><span>01</span><div><p className="eyebrow">New task</p><h2>Frame the decision</h2></div></header>
      ...
      <fieldset className="mode-field"><legend>Mode</legend><label><input type="radio" name="mode" checked={(form.mode ?? "fast") === "fast"} onChange={() => setForm({ ...form, mode: "fast" })}/> Fast Demo</label><label><input type="radio" name="mode" checked={form.mode === "verified"} onChange={() => setForm({ ...form, mode: "verified" })}/> Verified (limited)</label></fieldset>
