import type { TechScoutApi, TechScoutCreateRunRequest, TechScoutRunDetail } from "./contracts";
...
export const techScoutMockApi: TechScoutApi = {
  ...
  async createRun(request: TechScoutCreateRunRequest) {
    ...
    runs.set(id, run); polls.set(id, 0); return { data: run, location: `/api/v2/runs/${id}` };
  },
